...
export default {
  ...
  mounted() {
    this.http
      .get("/api/notice/search?type=green&handle=" + this.handle)
      .then(res => {
        console.log(res.data.push.last_page);
        this.lastpage = res.data.push.last_page;
        this.list = res.data.push.data;
      });
    this.http.get("/api/notice/count/detail?type=green").then(res => {
  ...
  methods: {
    change1(e) {
      this.http
        .get("/api/notice/search?type=green&page=" + e + "")
        .then(res => {
          this.list = res.data.push.data
        });
    },
    onLoad() {
      ...
      // 异步更新数据
      this.http
        .get("/api/notice/search?type=green&page=" + this.count + "")
        .then(res => {
    ...
    shenpi(id) {
      console.log(id);
      this.$router.push("/weishengsheng/" + id);
  ...
  watch: {
    value2(a, b) {
      ...
      if (a == "a") {
        this.bool = 0;
        this.handle = 9;
        this.http.get("/api/notice/search?type=green&handle=9").then(res => {
          console.log(res.data.push.last_page);
          this.lastpage = res.data.push.last_page;
          this.list = res.data.push.data;
        });
        ...
        this.bool = 0;
        this.handle = 0;
        this.http.get("/api/notice/search?type=green&handle=0").then(res => {
          console.log(res.data.push.last_page);
          this.lastpage = res.data.push.last_page;
          this.list = res.data.push.data;
        });
        ...
        this.bool = 0;
        this.handle = 1;
        this.http.get("/api/notice/search?type=green&handle=1").then(res => {
          console.log(res.data.push.last_page);
          this.lastpage = res.data.push.last_page;
          this.list = res.data.push.data;
        });
